...
test("Deve validar o fluxo de autenticação", async function () {
  ...
  await axios.post("http://localhost:3014/signup", input);
  const response = await axios.post("http://localhost:3014/login", input);
  ...
  const response2 = await axios.post("http://localhost:3014/verify", {
    token:
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6ImpvYW9AZ21haWwuY29tIiwiaWF0IjoxNjc3Njc1NjAwMDAwLCJleHBpcmVzSW4iOjEwMDAwMDB9.nPHGoaoMLLpmDS61-njfqX6G5ZvwT3Y5U71uOXGbRYY",
  });
